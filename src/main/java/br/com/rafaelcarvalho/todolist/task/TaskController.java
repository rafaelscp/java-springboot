...
public class TaskController {
    ...
    //http://localhost:8080/tasks/89213213-cdfdasa-2321421321
    @PutMapping("/{id}")
    public ResponseEntity update(@RequestBody TaskModel taskModel, HttpServletRequest request, @PathVariable UUID id) {
        ...
        if(!task.getIdUser().equals(idUser)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Usuário não tem permissão para aletrar está tarefa");
        }
